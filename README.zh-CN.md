...
- 新增**白名单**（`allowlist`）：在 WebUI 配置项里填写的词，即使命中检测规则也不会被打码（如填入 `114514`、账号名、房间号）。
- 纯数字密码检测阈值收紧到 **8 位以上**，减少 6 位纯数字（如 `114514`、邮编、编号）的误伤；常见的 6 位弱密码仍由弱密码词表拦截。


## v1.1.2 新增

- 新增**代码块跳过**：`skip_codeblock`（` ``` ` fenced 区块内不屏蔽）与 `skip_inline_code`（行内 `code` 不屏蔽），可分别开关。
